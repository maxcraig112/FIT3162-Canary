...
export const annotateHandler = {
    ...
    onBoundingBoxCreated(_points: Array<{ x: number; y: number }>, _label: string) {
        // no-op
        void _points;
        void _label;
    },
    ...
    confirmLabel(label: string, projectID?: string) {
            ...
            if (textObj) {
                ...
                if (meta) {
                    ...
                    if (kind === 'kp') {
                        ...
                        // placeholder for future bbox rename API
        ...
        if (currentTool === 'kp' && pendingKP) {
            ...
            const { group, annotation: ann } = keypointHandler.finalizeCreate(marker, x, y, label, projectID, currentImageID ?? undefined);
            canvasRef.add(group);
            canvasRef.requestRenderAll();
            ...
            if (meta && currentImageKey) {
                ...
                if (s) {
                    if (meta.kind === 'kp') {
                        ...
                        void boundingBoxHandler.deleteBoundingBox(meta.ann as BoundingBoxAnnotation, projectID);
                    }
                }
