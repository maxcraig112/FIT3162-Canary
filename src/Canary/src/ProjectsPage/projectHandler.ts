...
export async function renameProject(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Failed to rename project: ${res.status} ${res.statusText} - ${errorText}`,
    );
  }
...
export async function deleteProject(projectID: string): Promise<string> {
  ...
  if (!res.ok) {
    const errorText = await res.text();
    throw new Error(
      `Failed to delete project: ${res.status} ${res.statusText} - ${errorText}`,
    );
  }
  const responseText = await res.text();
  try {
...
export function handleSort(
  projects: Project[],
  sortKey: keyof Project = "projectName",
...
export async function handleNewProject(projectName: string): Promise<string> {
  ...
  if (!res.ok) {
    const errorText = await res.text();
    throw new Error(
      `Failed to create project: ${res.status} ${res.statusText} - ${errorText}`,
    );
  }
